...
```
This creates a table with 348 rows so there are 348 different attractions.
...
```
The number of visitors in 2022 is a numeric data type.
```{r Q2_part2}
visitors %>% summarise_all(class)
```
The summarise_all() function is used to display all different data types for each variable.
...
visitors %>% 
  ...
  head(n = 10)
```
The head() function shows the first 10 rows in the table.
## Question 4
...
visitors %>% 
  ...
  filter(n>=50)
```
By counting the number of attractions in each region, it introduces a variable n which can be used to filter out the regions that have more than 50 attractions. Scotland is the only region that meets the criteria.
...
visitors %>% 
  summarise(
    ...
    median_2021 = median(n_2021, na.rm = TRUE)
  )
```
...
NA stands for "Not Applicable" and this occurs because there are some missing values within the n_2021 column which the mean and median functions can't compute.
...
the argument na.rm is a logical data type, which decides whether NA values should be ignored or not. na.rm = TRUE means that the NA values are ignored.
...
This suggests that there are a few outliers that have a much larger number of visitors in 2022 compared to the rest of the data set which skews the mean.
...
This data shows that the inter-quartile range for Wales and Northern Ireland are inaccurate since there is a small data set.
